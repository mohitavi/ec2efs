 ...
 depends_on = [ 
  ...
  provisioner "remote-exec" {
      inline = [
        ...
        "sudo curl https://raw.githubusercontent.com/mohitavi/ec2efs/master/index.html > index.html",     
        "sudo cp index.html  /var/www/html/",
      ]
  }
}

resource "aws_s3_bucket" "task-2-s3bucketavi" {
depends_on = [
    null_resource.null-remote-1,    
  ]     
  bucket = "task-2-s3bucketavi"
  force_destroy = true
  acl    = "public-read"
  policy = <<POLICY
{
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "s3:*",
      "Resource": "arn:aws:s3:::task-2-s3bucketavi/*"
    }
  ]
}
POLICY
...
resource "aws_s3_bucket_object" "task-2-object" {
  depends_on = [ aws_s3_bucket.task-2-s3bucketavi,
                null_resource.null-remote-1,
               
 ]
     bucket = aws_s3_bucket.task-2-s3bucketavi.id
  key    = "one"
  source = "C:/Users/Avi/terra/terraform.jpg"
  etag = "C:/Users/Avi/terra/terraform.jpg"
  acl = "public-read"
  content_type = "image/jpg"
}

locals { 	
  s3_origin_id = "aws_s3_bucket.task-2-s3bucketavi.id"
}
...
resource "aws_cloudfront_distribution" "task-2-s3_distribution" {
  origin {
    domain_name = aws_s3_bucket.task-2-s3bucketavi.bucket_regional_domain_name
  ...
  logging_config {
    ...
    bucket          =  aws_s3_bucket.task-2-s3bucketavi.bucket_domain_name
